...
- **Heroes Configuration**: Per-roster hero pool managed via Dashboard Game Config (admin/coach with `manage_game_config`). Heroes have name, role (Duelist/Vanguard/Strategist), optional image upload, active/inactive toggle, and sortOrder. For Marvel Rivals rosters, ~50 default heroes auto-seed on first load (idempotent via a `heroes_defaults_seeded` setting flag set before insert to avoid concurrent double-seed). The schema also includes a forward-looking `game_heroes` join table (matchId, playerId, heroId, roundNumber, sortOrder) supporting future multi-hero per player per game records, ban & protect, and analytics.
