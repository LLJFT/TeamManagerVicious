...
- **Attendance Tracking**: Per-event attendance for players and staff with Present/Late/Absent statuses. Tracked in EventDetails page with dedicated attendance card. Backend: `attendance.staffId` field + `GET/POST /api/events/:eventId/attendance`. OrgDashboard shows attendance rates per roster. Players page attendance table includes staff rows with Player/Staff type badges.
...
- **Player Stats By Map**: Player Statistics page includes three tabs: Overall, By Mode, and By Map. Backend `/api/player-stats-summary` returns `statsByMap` alongside `statsByMode`, grouping stats by map name.
...
- **Test Data Defaults**: Per-roster auto-seeding includes 4 staff roles (Head Coach, Assistant Coach, Analyst, Manager), 3 chat channels (General, Strategy, Announcements), 5 availability slots, 4 roster roles, and 3 event categories with sub-types.
