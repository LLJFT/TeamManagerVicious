...
- **Game Templates** (super_admin): Reusable per-team configuration snapshots stored in `game_templates` (id varchar UUID, teamId, gameId, unique name+code per team, JSONB `config` containing gameModes/maps/heroes/statFields/scoreConfigs/eventCategories/availabilitySlots/opponents + singleModeGame). Pages: `/game-templates` (list with create/copy-code/delete) and `/game-templates/:id` (tabbed editor mirroring the Dashboard config tabs minus Users/Roles). Codes use the `<gameAbbrev3>-<6 alphanumeric>` format. CRUD endpoints (`/api/game-templates`) are super_admin-only; `GET /api/game-templates/by-code/:code` and `POST /api/game-templates/apply` (body `{templateId|code}`, query `gameId`+`rosterId`) require `manage_game_config` AND pass `verifyObjectScope` so callers can only apply to rosters they actually have access to. Apply runs in a single DB transaction: deletes roster-scoped (teamId+gameId+rosterId) game_modes, maps, stat_fields, heroes, event_categories, availability_slots, opponents, then re-inserts from the template config with tempId→UUID remapping (so foreign-key fields like `gameModeId` survive). Null-roster shared rows are NEVER touched. Players, games, results, events, attendance, and history are NEVER touched. The Dashboard "Reset Roster" tab includes an "Apply Game Template" card with code lookup, preview tally, and a confirmation dialog.
