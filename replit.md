...
- **Test Data**: 933 players (8 per roster: 2× Tank, 2× DPS, 2× Support, 2× Flex), 232 staff (2 per roster: Coach + Analyst), 5811 events (50 per roster: 20 scrims + 20 tournaments + 10 meetings), 23231 matches, 658880 stat entries, 69600 attendance records across all 116 rosters. 1511 user accounts (password "0000" for all player/staff accounts). 3 game modes per roster (Game Mode 1/2/3) with 3 maps each (Maps 1-9) and 4 stat fields per mode. 2 seasons per roster. All results normalized to lowercase (`win/loss/draw`), attendance to `attended/late/absent`. Per-roster auto-seeding includes 4 staff roles, 3 chat channels, 5 availability slots, 4 roster roles (Tank/DPS/Support/Flex), and 3 event categories with sub-types.
...
- **Sides Configuration**: Per-roster Side definitions (e.g. Attack/Defense) managed via Dashboard Game Config tab. Table `sides` (teamId, gameId, rosterId, name, sortOrder). Routes: `GET/POST /api/sides`, `PUT/DELETE /api/sides/:id` — all under `requireGameAccess`. GET auto-seeds Attack/Defense for any roster scope that has none. Sides are referenced by `game_rounds.sideId` (ON DELETE SET NULL).
- **Per-Mode Score Configuration**: Each `gameModes` row has `scoreType` ("numeric" or "rounds"), `maxScore` (cap for numeric), `maxRoundWins` (cap for rounds). Configurable per game mode in Dashboard Game Config tab. Drives multi-round inputs and clamps in EventDetails Add/Edit Game flow.
- **Multi-Round Game Scoring**: Table `game_rounds` (matchId FK→games cascade, roundNumber, sideId, teamScore, opponentScore). Routes: `GET /api/games/:matchId/rounds` (verifyObjectScope) and `PUT /api/games/:matchId/rounds` (bulk replace, edit_events permission). EventDetails Add New Game form renders up to 15 round blocks with side selector and score inputs bounded by the chosen mode's score config; existing games show a "Rounds" toggle that opens an inline editor below the games table.
